...
def forbid_runtime_start(monkeypatch):
    def unexpected_runtime(*_args, **_kwargs):
        pytest.fail("database maintenance commands must not start runtime services")

    monkeypatch.setattr("honeybuy_tg.app.run_bot", unexpected_runtime)
    monkeypatch.setattr("honeybuy_tg.app.start_metrics_exporter", unexpected_runtime)
...
def test_healthcheck_rejects_schema_mismatch_without_migrating(
    ...
    with sqlite3.connect(database_path) as database:
        ...
        database.execute(f"PRAGMA user_version = {database_version}")
    before = database_path.read_bytes()
    _configure_healthcheck(monkeypatch, tmp_path, database_path)
    ...
    assert database_path.read_bytes() == before
    with sqlite3.connect(database_path) as database:
        assert database.execute("PRAGMA user_version").fetchone()[0] == database_version
        assert database.execute("SELECT value FROM sentinel").fetchone()[0] == (
            "preserved"
        )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("DROP TABLE shopping_items", "shopping_items", id="missing-table"),
        pytest.param(
...
def test_healthcheck_rejects_integrity_failure(tmp_path, monkeypatch, capsys):
    ...
    with sqlite3.connect(database_path) as database:
        ...
        database.execute("INSERT INTO integrity_probe VALUES (-1)")
    _configure_healthcheck(monkeypatch, tmp_path, database_path)
...
def test_healthcheck_rejects_foreign_key_failure(tmp_path, monkeypatch, capsys):
    ...
    with sqlite3.connect(database_path) as database:
        ...
        database.execute("INSERT INTO fk_child VALUES (1, 999)")
    _configure_healthcheck(monkeypatch, tmp_path, database_path)
